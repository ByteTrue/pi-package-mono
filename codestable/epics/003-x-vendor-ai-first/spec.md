...
- `codestable/spec/pi-vendor/index.md`（本 epic 关闭时大幅重写）
- `codestable/spec/index.md`（包能力地图需同步）

被本 epic 作废的历史结论：

- closed epic `codestable/epics/001-x-vendor-dual-ui-manager/spec.md`（双 UI 单语义）
- closed epic `codestable/epics/002-x-vendor-web-productization/spec.md`（Web 产品化）
- closed issue `codestable/issues/016-x-vendor-web-modal-runtime.md` 至 `codestable/issues/024-x-web-visual-redesign.md`

历史材料本身不删，留在原处与 `codestable/archive/`；切片 4 统一标 superseded。
...
│     catalog  → 输出 active Pi 官方模板 JSON
│     discover → 按 effective API route bounded discovery
│     lint     → 本地 JSON / 结构检查
│     set-key  → 用户终端无回显、原子写入
└── TUI /vendor        ← 两条直线
      ├── Add provider
      └── Add model
```
...
- **AI 做判断，包做确定性读取与安全录入。** 官方模板选择、元数据搬运、字段编辑靠 AI；catalog 查询、protocol-aware discovery、本地 lint 与用户终端 key entry 由 bundled script 按需执行。运行中 `ctx.modelRegistry.refresh() + getError()` 是 TUI 写后的权威兼容性检查；脚本 `lint` 不冒充 Pi runtime oracle。AI 不凭记忆填字段——catalog 直接输出模板 JSON 供复制。
...
| # | Issue | 状态 |
|---|---|---|
| 1 | `codestable/issues/028-x-vendor-drop-web.md` — 删 Web 与死码 | closed |
| 2 | `codestable/issues/031-x-vendor-tui-linear.md` — TUI 两条直线 | closed |
| 3 | `codestable/issues/029-x-vendor-skill-verbs.md` — skill + 三个只读动词 | closed |
| 4 | `codestable/issues/030-x-vendor-spec-rewrite.md` — spec / superseded / README | closed |
| 5 | `codestable/issues/035-x-vendor-skill-script.md` — 常驻 tools 收口为 bundled script | closed |
| 6 | `codestable/issues/034-x-vendor-protocol-discovery.md` — 异构 API route discovery | closed |

历史依赖顺序是先删 Web、再重写 TUI、再加 Skill 与三个只读 verbs、最后收规格；关闭后又把 verbs 收口为单 bundled script，并补齐 protocol-aware discovery。最终 contract 以 issue 035 / 034 与本页关闭结论为准。
...
- 状态：`closed`
- 合并位置：`codestable/spec/pi-vendor/index.md`（AI-first 身份、Skill/script/TUI 路径、安全与 discovery 边界）；`codestable/spec/index.md`（包能力地图与当前方向）
- Vision：本 Epic 没有来源 Vision；当前 `codestable/vision/index.md` 仍是未确认空地图，因此不写入目标内容
- 保留材料：Talk、issue 执行记录与被 supersede 的旧 Web epic/issue

## 关闭结论

AI-first 转向完成。日常 CRUD 的唯一产品入口是 bundled Skill + AI edit；catalog/discover/lint/key entry 由 Skill 内单脚本按需承担，包不注册常驻 AI tool；另保留零模型冷启动 TUI。Web 产品承诺正式作废但历史证据保留。关闭后另行授权的 npm 发布已由后续 release 记录承担。

## 相关材料

- Talk：`codestable/talks/003-vendor-ai-first.md`
