...
- ~~选择目标与导入在 UI 上完全独立可验证（选目标不产生任何数据写入）~~ ✅ issue 002（单测断言选中真 profile 时 `playwright-cli` 未被调用）；
- ~~导入目的地硬闸有测试覆盖（拒绍真实 profile、拒绍被占用 profile）~~ ✅ 含真档案**子目录**也被拒；
- ~~撞锁报错能指名占用者~~ ✅；
- ~~README + `spec/pi-browser` 回写完成，note 008 与旧限制条目按新模型清算~~ ✅；
- ~~版本发布~~ ✅ tag `pi-browser-v0.2.0`，CI success，`+ @bytetrue/pi-browser@0.2.0`（provenance 已上透明日志）。
- **未满足（以故本 Epic 仍 open）**：真机证据“活档案上需鉴权页已登录且未破坏档案”——M0 得到的是反证（180s 超时 + 改写 111 个文件），而 /tmp 副本上是正证。默认副本路线已可交付；真 profile 要升为可用，需先定根因。
