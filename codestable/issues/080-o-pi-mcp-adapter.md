...
### 2026-09-17 优化（工具查看翻页，左右键切换，0.5.1）

- 解决痛点：工具量大时（如 cua-driver 54 个、全局 77 个）一股脑塞进 `ui.editor` 或超长列表阅读体验差。
- **分页工具浏览器 `runPaginatedToolBrowser`**：
  - 每页固定 10 个工具（Page X/Y，精确到 `1–10 of 54`）。
  - **支持左右键翻页**：`←` / `→`（及 `p`/`n`、`h`/`l`）无缝切页。
  - **上下键光标导航**：`↑` / `↓` 移动选择当前页 10 个工具。
  - **回车查看详情**：对选中工具按回车打开该工具的完整 Description 与 Parameters Schema（`renderTsShape`），关闭后无缝回到刚才所在的页码和光标位置。
  - **多处复用**：单 Server 工具浏览、全局 77 个工具浏览、关键词搜索结果浏览全部走此翻页器。
  - **高度恒定**：严格 16 行，每一行 visibleWidth 保证等于终端宽度，绝无错位或溢出崩溃。
- **验证**：86/86 单测全部通过（包含翻页状态与切页断言）。
