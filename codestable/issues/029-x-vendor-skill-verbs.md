...
- 隶属 epic：`codestable/epics/003-x-vendor-ai-first/spec.md`（切片 3）
...
Owner 验收后判定三个常驻 tool 属于过度设计；该运行时 surface 已由 `codestable/issues/035-x-vendor-skill-script.md` 取代。当前实现不注册 AI tools，catalog/discover/lint/set-key 均由 Skill bundled script 按需承担。本 issue 保留为当时实现历史，不再是现行接口合同。
