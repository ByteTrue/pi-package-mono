...
**归属：** 独立 issue。相关 spec：`codestable/spec/pi-background-terminal/index.md`；前序：071（单入口架构，本 issue 只翻参数默认值，不动架构）；讨论：本会话 2026-09-14（用户对"可选参数从不被使用"的实证追认 + 双默认设计确认 + 两个风险项授权解决）。
...
- 回写：`codestable/spec/pi-background-terminal/index.md` 全文重写（双默认语义、one-shot、文件上限、PATH 复刻、powershell 专属 hook）；README 使用路径与默认值说明更新。✅ 已完成。
