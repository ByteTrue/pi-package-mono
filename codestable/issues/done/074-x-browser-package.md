...
**归属：** 根 `issues/`（独立 issue）。相关 spec：`codestable/spec/index.md`（能力地图与架构落点新增一行，关闭时回写）。
...
- project spec：`codestable/spec/index.md`（包清单改七个、能力地图、使用路径、架构落点、当前边界、证据索引均新增 pi-browser）
- 新建 `codestable/spec/pi-browser/index.md`（定位 / `/browser` 表面 / 导入机制 / 配置四件套 / 会话语义 / 路径 / 明确不做 / 已知限制 / 验证 / 证据）
