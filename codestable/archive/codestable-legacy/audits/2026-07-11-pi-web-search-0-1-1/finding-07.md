...
`.codestable/issues/2026-07-11-web-search-budgets/` 在 orchestrator 增加 15 秒 provider attempt deadline + Promise race；provider 忽略 signal 时仍 fallback，external abort 在 race/catch/候选间均优先终止。最终 review：`subagent+ocr` passed。
