...
- Skill 明确 discovery 的证据等级：返回 id 是 positive evidence；配置存在但 route 未列出时给出待验证 warning，禁止仅用 list difference 断言模型不可用或自动删除/改路由。
...
- Skill 明确 list 结果的证据等级：缺集必须提醒验证，但禁止据此推断“上游不支持”或自动修改配置。
