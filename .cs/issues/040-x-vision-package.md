...
- 独立 issue。
- 相关 spec：`.cs/spec/index.md`（能力地图需新增一行）
...
- Pi 的 `processImage` / `detectSupportedImageMimeTypeFromFile` **没有从主入口导出**，不可静态 import（见 `.cs/notes/005`，import 不存在的符号是 link-time 错误，会让整个扩展加载失败）。自己做 magic-byte 嗅探。
...
- **不碰**：任何 Pi 原生工具的注册与行为；其他三个 workspace 包（`.cs/spec/index.md` 的"四包互不依赖"约定继续成立，`sniffMime` 从 `pi-image-gen` **复制**约 30 行而非跨包依赖）
...
- **信息安全性（Security）**：错误消息与工具输出不得包含 apiKey、resolved 凭据或 Authorization 头内容（继承 `.cs/spec/pi-vendor/index.md` 已有的凭据不外泄约定）。证据：单测断言错误文本不含注入的假 key；错误消息只来自本包常量 + 上游 status code。
...
- 新建 `.cs/spec/pi-vision/index.md`
- 根 `README.md`：包表格 + 本地开发示例 + test 命令
- notes：`.cs/notes/007-pi-clipboard-image-paths.md`（Pi ctrl+v 自落盘 + `images` 事件只有三个非交互产地）
...
- **回写位置**：稳定结论（不自动挑默认模型的理由、`input` hook 是死代码的事实、不用 Pi `SettingsManager` 写的理由、不做自研 TUI 分页的理由）已合入 `.cs/spec/pi-vision/index.md`。Pi 自身行为事实（ctrl+v 自落盘、`images` 产地）单独成 note 供其它包复用。中间过程（四次变异验证、两轮方案反复、自己 hook 的误报发现）留在本 issue，不搜 spec。
