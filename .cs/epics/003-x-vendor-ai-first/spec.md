...
- closed epic `.cs/epics/001-x-vendor-dual-ui-manager/spec.md`（双 UI 单语义）
- closed epic `.cs/epics/002-x-vendor-web-productization/spec.md`（Web 产品化）
- closed issue `.cs/issues/016-x-vendor-web-modal-runtime.md` 至 `.cs/issues/024-x-web-visual-redesign.md`
...
| # | Issue | 状态 |
|---|---|---|
| 1 | `.cs/issues/028-x-vendor-drop-web.md` — 删 Web 与死码 | closed |
| 2 | `.cs/issues/031-x-vendor-tui-linear.md` — TUI 两条直线 | closed |
| 3 | `.cs/issues/029-x-vendor-skill-verbs.md` — skill + 三个只读动词 | closed |
| 4 | `.cs/issues/030-x-vendor-spec-rewrite.md` — spec / superseded / README | closed |
| 5 | `.cs/issues/035-x-vendor-skill-script.md` — 常驻 tools 收口为 bundled script | closed |
| 6 | `.cs/issues/034-x-vendor-protocol-discovery.md` — 异构 API route discovery | closed |
...
- 状态：`closed`
- 合并位置：`.cs/spec/pi-vendor/index.md`（AI-first 身份、Skill/script/TUI 路径、安全与 discovery 边界）；`.cs/spec/index.md`（包能力地图与当前方向）
- Vision：本 Epic 没有来源 Vision；当前 `.cs/vision/index.md` 仍是未确认空地图，因此不写入目标内容
...
- Talk：`.cs/talks/003-vendor-ai-first.md`
