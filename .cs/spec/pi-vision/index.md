...
`@bytetrue/pi-vision` 让一个**没有视觉能力**的主力模型（如 DeepSeek、`qwen3.7-max`）也能"看图"：一个工具 `image_ask(paths, question)`，把本地图片交给你在 `models.json` 里已经配置好的另一个视觉模型，拿回文本答案。

Peer：`@earendil-works/pi-coding-agent` `>=0.79.10`；零 runtime 依赖。
...
- **不做 input hook**：已证明是死代码，见上文。
- **不收 http(s) URL**：只收本地路径；需要网络图片时模型自己先用别的工具下载。

## 统一语言

- **vision-capable model**：`models.json` 里 `input` 数组包含 `"image"` 的模型；`pi --list-models` 的 `images` 列显示 `yes`。
- **配置项 `pi-vision.model`**：`settings.json` 里 `"provider/model-id"` 形式的字符串，决定 `image_ask` 调用哪个模型；本包唯一持久化的配置。
