// mcp-trace.ts — opt-in metadata-only protocol tracing, ported from
// pi-mcp-adapter's mcp-trace.ts (MIT). Tracing must never change MCP
// request/response behavior: writers self-disable on any failure.
import { appendFile, mkdir, writeFile } from "node:fs/promises";
import { Buffer } from "node:buffer";
import { dirname, isAbsolute, resolve } from "node:path";
import type { Transport, JSONRPCMessage, MessageExtraInfo } from "@modelcontextprotocol/client";
import type { McpTraceSettings } from "./types.js";
