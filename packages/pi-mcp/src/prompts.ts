...
import type { McpConfig, PromptMetadata } from "./types.js";
import type { ServerManager } from "./server-manager.js";
import { isServerDisabled } from "./tool-naming.js";
import { formatPromptCommandName } from "./tool-naming.js";
import { isServerCacheValid, loadMetadataCache, reconstructPromptMetadata } from "./metadata-cache.js";

/** Adapter state surface this port needs (upstream used McpExtensionState). */
export interface PromptRuntime {
  manager: ServerManager;
  config: McpConfig;
  /** Server names whose live connection advertised a prompt list. */
...
const logger = { debug(message: string) { void message; } };
...
export function createPromptCommand(
  ...
  return {
    ...
    handler: async (args: string, ctx: ExtensionCommandContext) => {
      ...
      let client;
      try {
        client = await state.manager.clientFor(metadata.serverName, ctx.signal);
      } catch (error) {
        if (ctx.hasUI) {
          const message = `MCP server "${metadata.serverName}" is not available (${error instanceof Error ? error.message : String(error)}). Run /mcp reconnect ${metadata.serverName}.`;
      ...
      try {
        result = await client.getPrompt({
          name: dispatchMetadata.originalName,
...
function findLivePromptMetadata(
  ...
  if (!state.manager.isConnected(serverName)) return undefined;
  const meta = state.manager.metadataFor(serverName);
  return meta?.prompts.find(p => p.originalName === originalName);
}
 ...
 */
export function listAllPromptMetadata(state: PromptRuntime): PromptMetadata[] {
  const flat: PromptMetadata[] = [];
  for (const name of Object.keys(state.config.mcpServers)) {
    const meta = state.manager.metadataFor(name);
    if (meta) flat.push(...meta.prompts);
  }
