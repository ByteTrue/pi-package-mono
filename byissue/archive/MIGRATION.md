...
## 2026-09-17 byissue root rename

工作流切换到 ByIssue（bi），产物根目录从 `codestable/` 更名为 `byissue/`（`git mv` 整树搬移）。现行实体与 README 中的 `codestable/...` 引用统一更新为 `byissue/...`；冻结 archive 目录名 `archive/codestable-legacy/` 不变，其内部及历史记录中的 `.codestable` 旧引用不做历史改写。`skills-lock.json` 的 `codestable/CodeStable-Lite` 是 GitHub 来源，与本仓库路径无关，不动。变更记录：`byissue/issues/086-x-ff-codestable-dir-rename.md`。
