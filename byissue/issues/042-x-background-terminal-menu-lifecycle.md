...
- **回写**：稳定的生命周期、通知和用户菜单约束已回写 `byissue/spec/pi-background-terminal/index.md`，README 与 issue 执行记录同步。
- **遗留风险**：未执行真实交互式 Pi session smoke；后续若 Pi session shutdown 或 UI API 契约变化，应在 Pi 版本升级回归时补跑。

## 关闭回写
- 已将不限时但 session-scoped 清理、主动 kill 静默和 `/background` 菜单约束回写 `byissue/spec/pi-background-terminal/index.md`。
