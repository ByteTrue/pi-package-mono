...
- 改动：`src/browser-command.ts`（面板边框、`runManualSignIn`、`runLoginMenu`、Sessions 标签、导入 confirm 补一行提示）；`src/import/verify.ts`（新增 `LOGIN_SESSION`、`openLoginWindow`）；`README.md`、`byissue/spec/pi-browser/index.md` 同步。
...
**后续决策（同日）：** 用户反馈"同步一次就把手动登录全冲掉，体验太差"，为此实测了 `state-save`/`state-load`/`cookie-set` 与持久化语义（结论：冲掉是文件级覆盖的必然，要绕开得建 vault+replay 双层），用户判定成本不值——**明确接受重导后手动登录失效**，不做双层。依据与重评入口：`byissue/notes/008-chromium-profile-login-state-primitives.md`。
