...
该接口约束随后被 `byissue/issues/049-x-ff-simplify-web-search-schema.md` 取代：`retry_provider` 属于为限制模型行为而引入的非惯用 API，最终恢复为中性的 optional `provider` / `max_results`。本 issue 仅保留为决策历史。
