...
- 隶属 epic：`byissue/epics/003-x-vendor-ai-first/spec.md`（切片 1）
- 相关 spec：`byissue/spec/pi-vendor/index.md`（切片 4 重写，本切片不动）
...
本 repo `.pi/settings.json` 只加载本地 `pi-background-terminal`；`@bytetrue/pi-vendor` 是全局装的已发布 0.2.2。当前 Pi session 里的 `/vendor` 跑的是**带 Web 的published 版本**，不是本工作树。要真机验证得先改 settings 加载本地包并停用全局包（否则按 `byissue/notes/005-pi-local-package-loading.md` 会 tool 冲突），再 reload Pi。
