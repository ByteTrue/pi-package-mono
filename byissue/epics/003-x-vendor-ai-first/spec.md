...
- `byissue/spec/pi-vendor/index.md`（本 epic 关闭时大幅重写）
- `byissue/spec/index.md`（包能力地图需同步）

被本 epic 作废的历史结论：

- closed epic `byissue/epics/001-x-vendor-dual-ui-manager/spec.md`（双 UI 单语义）
- closed epic `byissue/epics/002-x-vendor-web-productization/spec.md`（Web 产品化）
- closed issue `byissue/issues/016-x-vendor-web-modal-runtime.md` 至 `byissue/issues/024-x-web-visual-redesign.md`

历史材料本身不删，留在原处与 `byissue/archive/`；切片 4 统一标 superseded。
...
| # | Issue | 状态 |
|---|---|---|
| 1 | `byissue/issues/028-x-vendor-drop-web.md` — 删 Web 与死码 | closed |
| 2 | `byissue/issues/031-x-vendor-tui-linear.md` — TUI 两条直线 | closed |
| 3 | `byissue/issues/029-x-vendor-skill-verbs.md` — skill + 三个只读动词 | closed |
| 4 | `byissue/issues/030-x-vendor-spec-rewrite.md` — spec / superseded / README | closed |
| 5 | `byissue/issues/035-x-vendor-skill-script.md` — 常驻 tools 收口为 bundled script | closed |
| 6 | `byissue/issues/034-x-vendor-protocol-discovery.md` — 异构 API route discovery | closed |
...
- 状态：`closed`
- 合并位置：`byissue/spec/pi-vendor/index.md`（AI-first 身份、Skill/script/TUI 路径、安全与 discovery 边界）；`byissue/spec/index.md`（包能力地图与当前方向）
- Vision：本 Epic 没有来源 Vision；当前 `byissue/vision/index.md` 仍是未确认空地图，因此不写入目标内容
...
- Talk：`byissue/talks/003-vendor-ai-first.md`
